...
function ReportViewModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="report-view-modal-title"
    >
      <div
        className="modal-content report-view-modal-content"
        onClick={(e) => e.stopPropagation()}
      >
        <h3 id="report-view-modal-title" className="modal-header">
          Lesson Report for {studentName} (Last {lessonCount} Conducted Lessons)
        </h3>
        {reportData.length > 0 ? (
            ...
            {reportData.map((item, index) => {
              ...
              return (
                <li key={index} className="report-item">
                  <span>
                    {formatFullDateForDisplay(lessonDateTime)} at {item.hour}{" "}
