...
import { 
  getWeekStartDate, getEndOfWeek, getStartOfMonth, getEndOfMonth, 
  getPreviousPeriod, parseTimeToDate
} from '../utils/dateUtils';

function StatisticsPage({ lessons, students, startOfWeekDay }: StatisticsPageProps) {
  const [statsView, setStatsView] = useState<'monthly' | 'weekly' | 'overall'>('monthly');
  const today = new Date();
  const currentMonthName = today.toLocaleDateString('en-US', { month: 'long' });
  
  const getPeriodStats = useCallback((periodStart: Date, periodEnd: Date) => {
    ...
    const income = paidLessonsInPeriod.reduce((sum, lesson) => sum + lesson.price, 0);
    const tips = paidLessonsInPeriod.reduce((sum, lesson) => sum + (lesson.tips || 0), 0);
  ...
  if (statsView === 'weekly') {
    ...
    const prevWeekStartDay = getPreviousPeriod(currentPeriodStart, 'week', startOfWeekDay);
    ...
    const prevMonthStartDay = getPreviousPeriod(currentPeriodStart, 'month', startOfWeekDay); // Pass start of current month
  ...
  const statsCurrentPeriod = (statsView === 'weekly' || statsView === 'monthly') ? getPeriodStats(currentPeriodStart!, currentPeriodEnd!) : {income:0, tips:0, count:0, paidCount:0};
  const statsPreviousPeriod = (statsView === 'weekly' || statsView === 'monthly') ? getPeriodStats(previousPeriodStart!, previousPeriodEnd!) : {income:0, tips:0, count:0, paidCount:0};
  
  const estimatedMonthlyIncomeValue = statsView === 'monthly' ? getEstimatedMonthlyIncome() : 0;
  const overallMonthlyIncomeValue = statsView === 'monthly' ? (statsCurrentPeriod.income + statsCurrentPeriod.tips) : 0;
  ...
  const paidLessonsThisMonthForBestClients = lessons.filter(l => {
    ...
    return l.isPaid && lessonDateOnly >= currentMonthStartForBestClients && lessonDateOnly <= currentMonthEndForBestClients;
  });

  const studentContributions: Record<string, number> = {};
  paidLessonsThisMonthForBestClients.forEach(l => {
    studentContributions[l.studentId] = (studentContributions[l.studentId] || 0) + l.price + (l.tips || 0);
  });
  ...
  const totalIncomeAllTime = lessons.filter(l => l.isPaid).reduce((sum, l) => sum + l.price + (l.tips || 0), 0);
  const totalLessonsAllTime = lessons.length;

  return (
    <div className="statistics-page" role="document" aria-labelledby="statistics-heading">
      <h2 id="statistics-heading">Income & Activity Statistics</h2>
      <div className="stats-view-toggle">
        <button 
          onClick={() => setStatsView('weekly')} 
          className={`view-toggle-button ${statsView === 'weekly' ? 'active' : ''}`}
        ...
        <button 
          ...
          className={`view-toggle-button ${statsView === 'monthly' ? 'active' : ''}`}
        ...
        <button 
          ...
          className={`view-toggle-button ${statsView === 'overall' ? 'active' : ''}`}
        ...
        {(statsView === 'weekly' || statsView === 'monthly') && (
          <>
            <div className="stat-card">
              ...
              <p className="income-amount">${statsCurrentPeriod.income.toFixed(2)}</p>
              <p className="stat-comparison">vs Prev. Period: ${statsPreviousPeriod.income.toFixed(2)} ({formatPercentageChange(statsCurrentPeriod.income, statsPreviousPeriod.income)})</p>
              <p className="stat-placeholder">[Graph: Income Trend]</p>
            </div>
            <div className="stat-card">
              <h3>Tips - {currentPeriodLabel}</h3>
              <p className="income-amount">${statsCurrentPeriod.tips.toFixed(2)}</p>
              <p className="stat-comparison">vs Prev. Period: ${statsPreviousPeriod.tips.toFixed(2)} ({formatPercentageChange(statsCurrentPeriod.tips, statsPreviousPeriod.tips)})</p>
            </div>
            <div className="stat-card">
              <h3>Lessons Scheduled - {currentPeriodLabel}</h3>
              <p className="lesson-count">{statsCurrentPeriod.count}</p>
              <p className="stat-comparison">vs Prev. Period: {statsPreviousPeriod.count} ({formatPercentageChange(statsCurrentPeriod.count, statsPreviousPeriod.count)})</p>
        ...
        {statsView === 'monthly' && (
          <>
            <div className="stat-card">
                ...
                <p className="income-amount">${overallMonthlyIncomeValue.toFixed(2)}</p>
            </div>
            <div className="stat-card">
              <h3>Estimated Monthly Income (All Scheduled Lesson Prices)</h3>
              <p className="income-amount">${estimatedMonthlyIncomeValue.toFixed(2)}</p>
            </div>
             <div className="stat-card">
                ...
                {topStudents.length > 0 ? (
                    ...
                    {topStudents.map(s => (
                        <li key={s.studentId}>{s.name}: ${s.total.toFixed(2)}</li>
                    ))}
                    </ul>
        ...
        {statsView === 'overall' && (
          <>
            <div className="stat-card">
              ...
              <p className="income-amount">${totalIncomeAllTime.toFixed(2)}</p>
            </div>
            <div className="stat-card">
